...
public class UU {
	...
	public static String randomString(int length) {
		...
		for (int i = 0; i < length; i++) {
			int ascii = UU.r.nextInt(26);
			ascii += UU.r.nextBoolean() ? 'a' : 'A';
			char c = (char) ascii;
			buf.append(c);
		}
		return buf.toString();
	...
	public static <T> Vector<Vector<T>> permutations(java.util.List<T> list) {
		return permutations(list, list.size());
	}
	...
	public static <T> void permutationsHelper(LinkedList<T> remaining,
			LinkedList<T> listSoFar, Vector<Vector<T>> results, int size) {
		if (listSoFar.size() == size) {
			results.add(new Vector<T>(listSoFar));
		} else {
			for (int i = 0; i < remaining.size(); i++) {
				listSoFar.add(remaining.removeFirst());
				permutationsHelper(remaining, listSoFar, results, size);
				remaining.add(listSoFar.removeLast());
			}
		}
	}
	...
	public static Color getRandomColor() {
		return new Color(Color.HSBtoRGB((float) UU.r.nextDouble(), 1.0f, 1.0f));
	}
	...
	public static <T> HashSet<T> getSubset(HashSet<T> source, int sizeOfSubset) {
		...
		while (subset.size() < sizeOfSubset) {
			subset.add(array[UU.r.nextInt(array.length)]);
		}
		return subset;
	}
